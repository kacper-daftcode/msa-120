...
class MiniMaxM3MoE(nn.Module):
    ...
    def __init__(
        ...
        # PATCH (NVFP4 on SM120): the ModelOpt NVFP4 MoE quant method reads
        # getattr(layer, "swiglu_limit") and gets None -> the marlin path raises
        # "SWIGLUOAI_UNINTERLEAVE requires clamp_limit". FusedMoE accepts these
        # kwargs but doesn't expose them on the quant-method layer object, so
        # force-set them on every candidate so the clamp is applied (correct
        # swigluoai, not plain SiLU).
        for _o in (
            self.experts,
            getattr(self.experts, "routed_experts", None),
            getattr(self.experts, "quant_method", None),
        ):
            if _o is not None:
                _o.swiglu_limit = config.swiglu_limit
                _o.swiglu_alpha = config.swiglu_alpha
                _o.swiglu_beta = config.swiglu_beta
...
class MiniMaxM3Model(nn.Module, EagleModelMixin):
    ...
    def get_expert_mapping(self) -> list[tuple[str, str, int, str]]:
        # The checkpoint stores experts as gate_proj / up_proj / down_proj
        # (e.g. ...block_sparse_moe.experts.0.gate_proj.weight after the
        # hf_to_vllm_mapper remap).  ``ckpt_*_proj_name`` MUST be those
        # checkpoint names so the weight_name substring match fires; the
        # internal shard ids (w1=gate, w2=down, w3=up) are generated by
        # make_expert_params_mapping itself.  Passing "w1"/"w2"/"w3" here
        # (the prior value) built weight_names like "experts.0.w1." that
        # never matched the checkpoint keys -> all 128 expert weights
        # silently stayed ZERO -> the MoE contributed nothing (the
        # "gibberish"/word-salad came from shared-experts + attention only).
...
class MiniMaxM3SparseForConditionalGeneration(
    ...
    hf_to_vllm_mapper = WeightsMapper(
        orig_to_new_prefix={
            ...
            "model.vision_tower.pre_layrnorm.": "vision_tower.vision_model.pre_layrnorm.",
            "model.vision_tower.post_layernorm.": "vision_tower.vision_model.post_layernorm.",
            "model.vision_tower.": "vision_tower.vision_model.",
            "model.multi_modal_projector.merge_linear_1.": "vision_tower.patch_merge_mlp.linear_1.",
            "model.multi_modal_projector.merge_linear_2.": "vision_tower.patch_merge_mlp.linear_2.",
            "model.multi_modal_projector.linear_1.": "vision_tower.multi_modal_projector.linear_1.",
            "model.multi_modal_projector.linear_2.": "vision_tower.multi_modal_projector.linear_2.",
        ...
        orig_to_new_substr={
            ...
            ".mlp.e_score_correction_bias": ".block_sparse_moe.e_score_correction_bias",
            ".mlp.fc1.": ".fc1.",
            ".mlp.fc2.": ".fc2.",
        },
    )
