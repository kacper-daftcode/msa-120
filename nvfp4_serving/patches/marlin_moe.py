...
class MarlinExpertsBase(mk.FusedMoEExpertsModular):
    def __init__(
        ...
        # PATCH (NVFP4 SM120): swiglu_limit can fail to reach the FusedMoEQuantConfig
        # for the ModelOpt NVFP4 path on this build, leaving gemm1_clamp_limit None ->
        # "SWIGLUOAI_UNINTERLEAVE requires clamp_limit". Fall back to an env value.
        if self.gemm1_clamp_limit is None:
            import os as _os
            _cl = _os.environ.get("VLLM_FORCE_SWIGLU_CLAMP_LIMIT")
            if _cl:
                self.gemm1_clamp_limit = float(_cl)
        ...
        import os as _os
        _swz = self.gemm1_clamp_limit is not None  # swigluoai active
        self.gemm1_alpha = (
            quant_config.gemm1_alpha if quant_config.gemm1_alpha is not None
            else (float(_os.environ["VLLM_FORCE_SWIGLU_ALPHA"]) if _swz and _os.environ.get("VLLM_FORCE_SWIGLU_ALPHA") else 1.0)
        )
        self.gemm1_beta = (
            quant_config.gemm1_beta if quant_config.gemm1_beta is not None
            else (float(_os.environ["VLLM_FORCE_SWIGLU_BETA"]) if _swz and _os.environ.get("VLLM_FORCE_SWIGLU_BETA") else 0.0)
        )
