...
def _unfused_swiglu_enabled() -> bool:
    """Gate the B2a un-fused swigluoai NVFP4 MoE path.

    Default ON (the whole point of this patched image is the correct MoE).
    Set VLLM_M3_UNFUSED_SWIGLUOAI_MOE=0 to fall back to the (broken) default
    kernel path for debugging.
    """
    import os

    # The cutlass W4A4 path is a sub-mode of the un-fused (raw-NVFP4-weights)
    # integration; if it is enabled the un-fused machinery (is_monolithic=False,
    # supports_internal_mk=True, raw-weight process_weights) must be active too.
    if os.environ.get("VLLM_M3_CUTLASS_FP4_MOE", "0") not in ("0", "false", ""):
        return True
    return os.environ.get("VLLM_M3_UNFUSED_SWIGLUOAI_MOE", "1") not in ("0", "false", "")


def _cutlass_fp4_moe_enabled() -> bool:
    """Gate the W4A4 batched-cutlass NVFP4 MoE fast path.

    When set, process_weights builds BATCHED (num_groups=E) mma weight scales and
    apply() runs ONE multi-group group_gemm_nvfp4_nt_groupwise per projection on a
    static [E, C] routing table (graph-capture safe), instead of the per-expert
    single-group Python loop.  Both paths are numerically bit-identical (proven by
    cutlass_fp4_moe/test_batched_graphsafe.py: vs per-expert loop = 0.0).

    Implies the un-fused (raw-NVFP4-weights) integration; only the kernel differs.
    """
    import os

    return os.environ.get("VLLM_M3_CUTLASS_FP4_MOE", "0") not in ("0", "false", "")


def _cutlass_fp4_capacity() -> "int | None":
    """Static per-expert capacity C for the graph-safe batched path.

    None -> worst-case pad4(T*k) per call (always correct, but C grows with the
    captured batch -> bigger buffers).  An explicit value caps each expert's row
    count for a tighter static buffer (overflow tokens are dropped, matching
    standard capacity-MoE semantics).  Set VLLM_M3_CUTLASS_FP4_CAP=<int>.
    """
    import os

    v = os.environ.get("VLLM_M3_CUTLASS_FP4_CAP", "")
    if v.strip() in ("", "0"):
        return None
    return int(v)
...
class ModelOptFp8MoEMethod(FusedMoEMethodBase):
    ...
    def get_fused_moe_quant_config(self, layer: RoutedExperts) -> FusedMoEQuantConfig:
        ...
        return make_fp8_moe_quant_config(
            ...
            a1_scale=a1_scale,
            a2_scale=a2_scale,
            swiglu_limit=(getattr(layer, "swiglu_limit", None) if getattr(layer, "swiglu_limit", None) is not None else getattr(getattr(self, "moe", None), "swiglu_limit", None)),
        )
...
class ModelOptNvFp4FusedMoE(FusedMoEMethodBase):
    ...
    def __init__(
        ...
        # B2a un-fused path runs its own per-expert NVFP4 GEMM loop and never
        # touches a registered MoE backend kernel; on SM120 backend selection
        # raises ("No NvFp4 MoE backend supports the deployment configuration"),
        # so swallow it and leave the kernel fields unused.
        try:
            ...
            if not _unfused_swiglu_enabled():
                raise
            self.nvfp4_backend = None
            self.experts_cls = None

        self.use_global_sf = (
            is_global_sf_supported_for_nvfp4_backend(self.nvfp4_backend)
            if self.nvfp4_backend is not None
            else False
        )

    @property
    def is_monolithic(self) -> bool:
        # The B2a un-fused path is a modular (non-monolithic) method that runs
        # its own apply(); experts_cls may be None so the base property (which
        # calls self.experts_cls.is_monolithic()) would crash.
        if _unfused_swiglu_enabled():
            return False
        return super().is_monolithic

    @property
    def supports_internal_mk(self) -> bool:
        # We "own" the kernel: process_weights builds per-expert mma scales and
        # apply() runs the un-fused loop directly.  Returning True makes the
        # runner's maybe_init_modular_kernel() skip building a modular kernel
        # (which would crash since experts_cls/moe_kernel are None).
        if _unfused_swiglu_enabled():
            return True
    ...
    def process_weights_after_loading(self, layer: RoutedExperts) -> None:
        ...
        # ---- B2a UN-FUSED swigluoai NVFP4 path (SM120 correctness) ----------
        # Replaces the numerically-broken forced-marlin MoE.  We KEEP the raw
        # NVFP4 weights (E2M1 packed data, E4M3 LINEAR block-16 scales, fp32
        # global scales) exactly as loaded, precompute per-expert mma weight
        # scales, and run the validated per-expert single-group group_gemm loop
        # (unfused_swigluoai_nvfp4_moe) in apply().  See
        # docs/B2a_UNFUSED_NVFP4_MOE.md and nvfp4_serving/unfused_moe/.
        if _unfused_swiglu_enabled():
            self._unfused_process_weights_after_loading(layer)
            return
    ...
    # ------------------------------------------------------------------ #
    # B2a UN-FUSED swigluoai NVFP4 MoE (SM120 correctness path)
    # ------------------------------------------------------------------ #
    def _unfused_process_weights_after_loading(self, layer: RoutedExperts) -> None:
        """Keep raw NVFP4 weights, precompute per-expert mma block scales.

        Registered shapes (vLLM create_weights, after TP intermediate-shard):
          w13_weight        [E, 2*I, H//2]   uint8 (E2M1 packed, gate;up stacked)
          w13_weight_scale  [E, 2*I, H//16]  float8_e4m3fn (LINEAR block-16)
          w13_weight_scale_2[E, 2]           float32 (per-shard global; gate==up)
          w2_weight         [E, H, I//2]     uint8 (E2M1 packed, down)
          w2_weight_scale   [E, H, I//16]    float8_e4m3fn
          w2_weight_scale_2 [E]              float32
        These match unfused_swigluoai_nvfp4_moe's expectations exactly except
        w13_weight_scale_2 needs [:, 0] (single gscale, gate==up).
        """
        from nvfp4_unfused_moe import build_expert_weight_scales_mma
        ...
        # Collapse the per-shard w13 global scale to a single scalar per expert.
        w13_scale_2 = layer.w13_weight_scale_2
        if w13_scale_2.dim() == 2:
            if not torch.allclose(w13_scale_2[:, 0], w13_scale_2[:, 1]):
                logger.warning_once(
                    "w1_weight_scale_2 != w3_weight_scale_2; using gate's."
                )
            w13_scale_2 = w13_scale_2[:, 0].contiguous()
        replace_parameter(layer, "w13_weight_scale_2", w13_scale_2)

        # One-time shape/dtype dump for the report (rank 0 only).
        try:
            from vllm.distributed import get_tensor_model_parallel_rank
            _rank = get_tensor_model_parallel_rank()
        except Exception:
            _rank = 0
        if _rank == 0 and not getattr(ModelOptNvFp4FusedMoE, "_unfused_dumped", False):
            ModelOptNvFp4FusedMoE._unfused_dumped = True
            logger.info(
                "[B2a unfused MoE] E=%d 2I=%d I=%d H=%d | "
                "w13_weight=%s/%s w13_weight_scale=%s/%s w13_weight_scale_2=%s/%s | "
                "w2_weight=%s/%s w2_weight_scale=%s/%s w2_weight_scale_2=%s/%s",
                E, twoI, I, H,
                tuple(layer.w13_weight.shape), layer.w13_weight.dtype,
                tuple(layer.w13_weight_scale.shape), layer.w13_weight_scale.dtype,
                tuple(w13_scale_2.shape), w13_scale_2.dtype,
                tuple(layer.w2_weight.shape), layer.w2_weight.dtype,
                tuple(layer.w2_weight_scale.shape), layer.w2_weight_scale.dtype,
                tuple(layer.w2_weight_scale_2.shape), layer.w2_weight_scale_2.dtype,
            )

        w13_scale_list = [layer.w13_weight_scale[e].contiguous() for e in range(E)]
        w2_scale_list = [layer.w2_weight_scale[e].contiguous() for e in range(E)]

        if _cutlass_fp4_moe_enabled():
            # W4A4 BATCHED path: build ONE (num_groups=E) mma weight-scale buffer
            # per projection (the kernel reads group i at stride i*sf_n*sf_k).
            from batched_graphsafe_moe import build_batched_weight_scale_mma
            layer._cutlass_w13_sf_mma = build_batched_weight_scale_mma(
                w13_scale_list, twoI, H
            )
            layer._cutlass_w2_sf_mma = build_batched_weight_scale_mma(
                w2_scale_list, H, I
            )
            layer._cutlass_I = I
            layer._cutlass_H = H
            # Group-plans are cached per static capacity C in apply().
            layer._cutlass_plan_cache = {}
            if _rank == 0:
                logger.info(
                    "[cutlass W4A4 MoE] batched grouped-GEMM path ENABLED "
                    "(E=%d 2I=%d I=%d H=%d, cap=%s)",
                    E, twoI, I, H, _cutlass_fp4_capacity(),
                )
            self.moe_kernel = None
            return

        # Precompute per-expert mma weight scales (swizzle -> mma 6D layout).
        layer._unfused_w13_sf_mma = build_expert_weight_scales_mma(
            w13_scale_list, twoI, H
        )
        layer._unfused_w2_sf_mma = build_expert_weight_scales_mma(
            w2_scale_list, H, I
        )
        # No kernel needed; apply() runs the per-expert loop directly.
        self.moe_kernel = None
    ...
    def _unfused_apply(
        ...
        from nvfp4_unfused_moe import unfused_swigluoai_nvfp4_moe
        ...
        if _cutlass_fp4_moe_enabled():
            out = self._cutlass_apply(
                layer, x, topk_weights, topk_ids, float(alpha), float(limit)
            )
        else:
            # topk_weights are renormalized (routed_scaling_factor applied to the
            # OUTPUT by the runner since apply_routed_scale_to_output=True), so we
            # pass them through unchanged and return only routed-expert output.
            out = unfused_swigluoai_nvfp4_moe(
                x,
                layer.w13_weight,
                ...
                alpha=float(alpha),
                limit=float(limit),
                w13_sf_mma=layer._unfused_w13_sf_mma,
                w2_sf_mma=layer._unfused_w2_sf_mma,
            )

        # One-shot in-process numeric self-check vs a bf16-dequant reference on
        # the EXACT live tokens/routing (gated; rank0, first call).  Proves the
        # wired path matches the ~0.13 NVFP4 noise floor (or exposes a wiring
        # bug).  Enable with VLLM_M3_UNFUSED_SELFCHECK=1.  Gate on REAL tokens
        # (1 <= T < 1000 with nonzero activation) so it never fires on a
        # zero/dummy warmup batch (which would misleadingly read 0.0000).
        #
        # CRITICAL: every guard below must be evaluated WITHOUT a device->host
        # sync (no bool(tensor), .item()) until we are certain we are NOT inside
        # a CUDA-graph capture, otherwise the capture is invalidated.  We gate
        # first on python-only conditions (env flag, python-int x.shape[0], the
        # class-level once-flag) and only THEN touch tensor values.
        import os
        _want = (
            os.environ.get("VLLM_M3_UNFUSED_SELFCHECK", "0") == "1"
            and (1 <= x.shape[0] < 1000)
            and not getattr(ModelOptNvFp4FusedMoE, "_unfused_selfcheck_done", False)
            and not torch.cuda.is_current_stream_capturing()
        )
        # Real-token check (the device sync) only happens once capture is ruled
        # out and the env flag is on -> never runs on the hot capture/decode path.
        if _want and bool(x.abs().sum() > 0) and bool((topk_ids >= 0).any()):
            try:
                from vllm.distributed import get_tensor_model_parallel_rank
                _rank = get_tensor_model_parallel_rank()
            except Exception:
                _rank = 0
            if _rank == 0:
                ModelOptNvFp4FusedMoE._unfused_selfcheck_done = True
                try:
                    self._unfused_selfcheck(
                        layer, x, topk_weights, topk_ids, out,
                        float(alpha), float(limit),
                    )
                except Exception as ex:  # noqa: BLE001
                    logger.warning("[B2a selfcheck] failed: %r", ex)
        return out
    ...
    def _cutlass_apply(
        ...
        """W4A4 batched-cutlass path: ONE multi-group group_gemm per projection.

        Two sub-modes (both REAL FP4 W4A4 compute, both bit-identical to the
        per-expert loop):
          * DYNAMIC (VLLM_M3_CUTLASS_FP4_DYNAMIC=1, the eager default): one
            grouped GEMM over only the ACTIVE experts, buffers sized to the live
            token count.  Correct for ANY batch size (no capacity drop), but uses
            nonzero/dynamic shapes -> NOT CUDA-graph capturable (eager only).
          * STATIC (graph mode): fixed [E, C] routing table -> graph-capturable,
            but every expert processes exactly C rows and tokens beyond C are
            dropped (so C must cover the max tokens-per-expert in the batch)."""
        import os as _os
        ...
        if _os.environ.get("VLLM_M3_CUTLASS_FP4_DYNAMIC", "0") not in ("0", "false", ""):
            from batched_dynamic_moe import batched_swigluoai_nvfp4_moe
            return batched_swigluoai_nvfp4_moe(
                x,
                layer.w13_weight, layer.w13_weight_scale, layer.w13_weight_scale_2,
                layer.w2_weight, layer.w2_weight_scale, layer.w2_weight_scale_2,
                topk_ids, topk_weights,
                activation="swigluoai", alpha=alpha, limit=limit,
            )

        from batched_graphsafe_moe import (
            batched_graphsafe_swigluoai_nvfp4_moe,
            _GroupPlan,
            _pad4,
        )

        cap = _cutlass_fp4_capacity()
        C = _pad4(int(cap)) if cap is not None else _pad4(T * k)

        # Cache the per-(C) static GroupPlans (m_indptr + SF scatter offsets).
        cache = layer._cutlass_plan_cache
        plans = cache.get(C)
        if plans is None:
            plan13 = _GroupPlan(E, C, H, x.device)
            plan2 = _GroupPlan(E, C, I, x.device)
            plans = (plan13, plan2)
            cache[C] = plans
        plan13, plan2 = plans

        return batched_graphsafe_swigluoai_nvfp4_moe(
            x,
            layer.w13_weight, None, layer.w13_weight_scale_2,
            layer.w2_weight, None, layer.w2_weight_scale_2,
            topk_ids, topk_weights,
            activation="swigluoai", alpha=alpha, limit=limit,
            w13_sf_mma=layer._cutlass_w13_sf_mma,
            w2_sf_mma=layer._cutlass_w2_sf_mma,
            plan13=plan13, plan2=plan2,
            expert_capacity=cap,
        )

    @staticmethod
    def _unfused_selfcheck(layer, x, topk_weights, topk_ids, out, alpha, limit):
        import torch as _t
        from nvfp4_unfused_moe import swigluoai as _swig

        _E2M1 = _t.tensor(
            [0., .5, 1, 1.5, 2, 3, 4, 6, -0., -.5, -1, -1.5, -2, -3, -4, -6],
            dtype=_t.float32, device=x.device,
        )
        ...
        def _unpack(p):
            ...
            lo = p & 0x0F
            hi = (p >> 4) & 0x0F
            o = _t.empty(p.shape[0], p.shape[1] * 2, dtype=_t.float32, device=p.device)
            o[:, 0::2] = _E2M1[lo]
            o[:, 1::2] = _E2M1[hi]
            return o
        ...
        E = layer.w13_weight.shape[0]
        T, H = x.shape
        ref = _t.zeros(T, H, dtype=_t.float32, device=x.device)
        for e in range(E):
            sel = topk_ids == e
            if not sel.any():
                continue
            tok, slot = sel.nonzero(as_tuple=True)
            w13 = _deq(layer.w13_weight[e], layer.w13_weight_scale[e],
                       layer.w13_weight_scale_2[e])
            w2 = _deq(layer.w2_weight[e], layer.w2_weight_scale[e],
                      layer.w2_weight_scale_2[e])
            gate_up = x[tok].float() @ w13.t()
            act = _swig(gate_up, alpha=alpha, limit=limit)
            y = act @ w2.t()
            ref.index_add_(0, tok, y * topk_weights[tok, slot].float().unsqueeze(-1))
        rel = ((out.float() - ref).pow(2).mean().sqrt()
               / (ref.pow(2).mean().sqrt() + 1e-20)).item()
        logger.info(
            "[B2a selfcheck] live unfused-nvfp4 vs bf16-dequant ref: rel RMS = "
            "%.4f  (T=%d H=%d E=%d; expect ~0.13 NVFP4 noise floor)",
            rel, T, H, E,
        )
    ...
    def apply(
        ...
        assert not self.is_monolithic
        if _unfused_swiglu_enabled():
            # shared_experts are applied by the runner separately; return only
            # the combined routed-expert output (runner scales by routed_scale).
            return self._unfused_apply(layer, x, topk_weights, topk_ids)
        assert self.moe_kernel is not None
        return self.moe_kernel.apply(
            x,
            layer.w13_weight,
...
class ModelOptMxFp8FusedMoE(FusedMoEMethodBase):
    ...
    def get_fused_moe_quant_config(
        ...
        return make_fp8_moe_quant_config(
            ...
            swiglu_limit=(getattr(layer, "swiglu_limit", None) if getattr(layer, "swiglu_limit", None) is not None else getattr(getattr(self, "moe", None), "swiglu_limit", None)),
            gemm1_alpha=(getattr(layer, "swiglu_alpha", None) if getattr(layer, "swiglu_alpha", None) is not None else getattr(getattr(self, "moe", None), "swiglu_alpha", None)),
            gemm1_beta=(getattr(layer, "swiglu_beta", None) if getattr(layer, "swiglu_beta", None) is not None else getattr(getattr(self, "moe", None), "swiglu_beta", None)),
        )
