...
sm120_fmha_perhead_bf16(
    const __nv_bfloat16 * __restrict__ Q,
    const __nv_bfloat16 * __restrict__ K,
    const __nv_bfloat16 * __restrict__ V,
    __nv_bfloat16 * __restrict__ O,
    float * __restrict__ LSE,
    const int * __restrict__ block_ids,   // see layouts below (-1=pad)
    int topk,
    int seq_len_q,
    int seq_len_k,
    int num_heads_q,
    ...
    float softmax_scale,
    int causal,
    int per_query,            // 0: per-tile (block_ids[m_blk,topk]); 1: per-query row
    int blk_kv,               // KV block size in tokens (64 or 128)
    // ---- per-(query,head) generalization (per-query path only) ----
    // The selected-block list for query row `qrow` of THIS head (blockIdx.y=hq)
    // starts at:  block_ids + hq*bids_head_stride + qrow*bids_row_stride
    // and spans `topk` ints. Layout mapping (host sets the two strides):
    //   * head-agnostic per-query [seq_q,topk]:  row_stride=topk,    head_stride=0
    //   * per-head [seq_q,Hq,topk]:              row_stride=Hq*topk, head_stride=topk
    //   * per-head [Hq,seq_q,topk]:              row_stride=topk,    head_stride=seq_q*topk
    // Per-tile mode (per_query=0) ignores both and keeps the head-agnostic
    // block_ids[m_blk*topk + t] indexing for backward compat.
    int bids_row_stride,
    int bids_head_stride)
{
    ...
    const int *blk_row = block_ids + m_blk * topk; // per-tile row (head-agnostic)

    // Per-query base for THIS head: block_ids[hq*head_stride + qrow*row_stride + t].
    // bids_qh(qrow) returns the start of qrow's top-k list for head hq.
    const int *bids_head = block_ids + hq * bids_head_stride;
    #define BIDS_QH(qrow) (bids_head + (qrow) * bids_row_stride)
    ...
    if (per_query) {
        ...
        if (sub == 0) {                            // 1 lane per (r0,r1) pair
            const int *row0 = BIDS_QH(my_r0);
            const int *row1 = BIDS_QH(my_r1);
            #pragma unroll 1
    ...
    for (int t = 0; t < n_iter; t++) {
      ...
      for (int sub_tile = 0; sub_tile < n_sub; sub_tile++) {
        ...
        if (per_query) {
            ...
            const int *row0 = BIDS_QH(q_r0);
            const int *row1 = BIDS_QH(q_r1);
            #pragma unroll 1
            for (int tt = 0; tt < topk; tt++) {
                if (q_r0 < seq_len_q && row0[tt] == kb) sel0 = true;
                if (q_r1 < seq_len_q && row1[tt] == kb) sel1 = true;
            }
            if (!sel0) {
                #pragma unroll
...
// Core launcher. block_ids strides resolved by callers below.
//   per_query=0 (per-tile): block_ids[m_blk*topk + t], head-agnostic.
//   per_query=1 (per-query/per-head): list at
//       block_ids + hq*bids_head_stride + qrow*bids_row_stride, span topk.
static std::vector<torch::Tensor> launch_perhead(
    torch::Tensor q, torch::Tensor k, torch::Tensor v,
    torch::Tensor block_ids, int topk, int per_query,
    int bids_row_stride, int bids_head_stride,
    double softmax_scale, bool causal, int64_t blk_kv)
{
    ...
    cudaFuncSetAttribute(sm120_fmha_perhead_bf16,
        cudaFuncAttributeMaxDynamicSharedMemorySize, smem_bytes);

    auto stream = at::cuda::getCurrentCUDAStream();
    sm120_fmha_perhead_bf16<<<grid, block, smem_bytes, stream>>>(
        ...
        bids_row_stride, bids_head_stride);
    return {o, lse};
}

// Backward-compatible entrypoint. block_ids 2D:
//   [num_m_blocks,topk] -> per-tile (head-agnostic)
//   [seq_q,topk]        -> per-query (head-agnostic)
    ...
    torch::Tensor block_ids, double softmax_scale, bool causal, int64_t blk_kv)
{
    ...
    // Auto-detect: [num_m_blocks,topk] (per-tile) or [seq_q,topk] (per-query).
    int per_query;
    ...
    // head-agnostic per-query: row_stride=topk, head_stride=0
    return launch_perhead(q, k, v, block_ids, topk, per_query,
                          topk, 0, softmax_scale, causal, blk_kv);
}

// Per-(query,head) entrypoint. block_ids 3D, one top-k list per (query,head):
//   [seq_q, Hq, topk]  (golden layout)  -> row_stride=Hq*topk, head_stride=topk
//   [Hq, seq_q, topk]                   -> row_stride=topk,    head_stride=seq_q*topk
// Also accepts 2D [seq_q,topk] / [num_m_blocks,topk] (delegates, head-agnostic).
std::vector<torch::Tensor> forward_sparse_perhead_bf16(
    torch::Tensor q, torch::Tensor k, torch::Tensor v,
    torch::Tensor block_ids, double softmax_scale, bool causal, int64_t blk_kv)
{
    TORCH_CHECK(blk_kv == 64 || blk_kv == 128, "blk_kv must be 64 or 128");
    TORCH_CHECK(q.is_cuda() && q.dtype() == torch::kBFloat16 && q.dim() == 3 && q.size(2) == 128,
                "q must be CUDA bf16 [Sq,Hq,128]");
    TORCH_CHECK(block_ids.is_cuda() && block_ids.dtype() == torch::kInt32,
                "block_ids must be CUDA int32");

    if (block_ids.dim() == 2)            // delegate to head-agnostic path
        return forward_sparse_bf16(q, k, v, block_ids, softmax_scale, causal, blk_kv);

    TORCH_CHECK(block_ids.dim() == 3, "block_ids must be 2D or 3D");
    q = q.contiguous(); k = k.contiguous(); v = v.contiguous();
    block_ids = block_ids.contiguous();

    const int seq_q = q.size(0), num_heads_q = q.size(1);
    const int d0 = block_ids.size(0), d1 = block_ids.size(1), topk = block_ids.size(2);

    int row_stride, head_stride;
    if (d0 == seq_q && d1 == num_heads_q) {          // [seq_q, Hq, topk]
        row_stride  = num_heads_q * topk;
        head_stride = topk;
    } else if (d0 == num_heads_q && d1 == seq_q) {   // [Hq, seq_q, topk]
        row_stride  = topk;
        head_stride = seq_q * topk;
    } else {
        TORCH_CHECK(false, "3D block_ids must be [seq_q,Hq,topk] or [Hq,seq_q,topk]");
        row_stride = head_stride = 0;
    }

    return launch_perhead(q, k, v, block_ids, topk, /*per_query=*/1,
                          row_stride, head_stride, softmax_scale, causal, blk_kv);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("forward_sparse_perhead", &forward_sparse_perhead_bf16,
          "SM120 block-sparse FA2 forward, per-(query,head) selection (BF16)",
          pybind11::arg("q"), pybind11::arg("k"), pybind11::arg("v"),
          pybind11::arg("block_ids"), pybind11::arg("softmax_scale"),
          pybind11::arg("causal") = false, pybind11::arg("blk_kv") = 64);
}
